...
- **현상:**
    ...
    - LOSS에서 저 항은 q(z|x)와 p(z)의 차이를 줄여야하는데, p(z)를 우리가 다른거없이 쌩으로 가우시안 노이즈로 가정했고, q(z|x)는 x를 보고 z를 뱉는 분포인데, 얘를 그냥 x는 주든말든 p(z)랑 똑같이 만들면 KL DIV 특성상 그냥 0이 되니까 그렇게 해버림.
    - 
