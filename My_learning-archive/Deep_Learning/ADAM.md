...
첫번째 레이어에서 z = wx + b 계산하고, (물론 BN 과정에서 b는 의미를 잃으므로 아예 정의하지 않는 것이 정석)
...
두번째 레이어에서도 똑같이 z = wx + b계산하고, (물론 BN 과정에서 b는 의미를 잃으므로 아예 정의하지 않는 것이 정석)
...
여기서 w는 가중치 w, scaling parameter (gamma), shift parameter(beta)가 모두 한 column에 나열된 M(가중치 수) + 2N(뉴런 수) 차원의 column vector
