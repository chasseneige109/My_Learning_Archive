...
### 3. 해상도가 바뀔 때: 보간법 (Interpolation)

만약 $224 \times 224$ 이미지(패치 196개)로 학습한 모델을, $384 \times 384$ 이미지(패치 576개)에 적용(Fine-tuning)하려면 어떻게 할까요? 학습된 PE는 196개뿐인데 개수가 안 맞습니다.

이때는 **2D 보간법 (Bicubic Interpolation)**을 사용합니다.

1. 학습된 1D PE(196개)를 다시 $14 \times 14$ 형태의 2D 그리드로 복원합니다.
    
2. 이 그리드를 이미지 늘리듯이 $24 \times 24$ 크기로 쭉 늘립니다(Interpolate).
    
3. 늘어난 값을 다시 1D로 펴서 576개의 새로운 PE로 사용합니다.
    

이 방식이 가능한 이유도, **PE가 이미 2D 공간 정보를 담고 있기 때문**입니다.