...
- **인코더 (Encoder):** Seq2Seq 모델의 **입력 처리 부분**을 담당합니다. 입력 시퀀스를 읽어 최종적으로 하나의 문맥 벡터($\mathbf{C}$)로 압축하는 역할을 합니다.
...
- 지금 내 머릿속의 생각($\mathbf{h}_t$)이 사전(Dictionary)에 있는 10,000개의 단어 중 누구와 가장 비슷한지 (내적이 큰지) 점수를 매기는 채점 과정.

h_t: 지금까지 읽은 모든 단어의 맥락(Context)을 256개의 실수로 압축한 정보 (1 x 256)

W_proj : 단어 10000개에 대해, 각 단어 당 256개의 문맥 정보를 담고 있는 행렬 ( 256 x 10000 )
...
우리는 Teacher Forcing을 "정답을 넣어주는 것"이라고 정의했지만, 강의자는 이를 **"일종의 치트(Cheat)"**라고 표현하며 실무적 팁을 던집니다.

- **강의 내용:**
 
- 학습 시 정답만 넣어주면(Teacher Forcing), 나중에 추론할 때(자기가 만든 걸 넣어야 할 때) 모델이 당황할 수 있다.

- **팁:** 실제로는 SGD 학습 중에 **"가끔은 정답 대신 모델이 뱉은 랜덤한 단어를 다음 입력으로 넣어주는"** 방식(Scheduled Sampling 개념)이 효과적일 수 있다는 점을 스치듯 언급했습니다.

# 이미지 캡셔닝 (CNN Encoder) 📷

우리는 텍스트-텍스트 모델만 이야기했지만, 강의자는 인코더의 **유연성(Modularity)**을 강조합니다.

- **강의 내용:**
    
    - 인코더 자리에 LSTM 대신 **CNN(이미지 처리 신경망)**을 끼워 넣고, 그 출력을 디코더 LSTM에 넣으면 그게 바로 **'이미지 캡셔닝(Image Captioning)'** 모델이 된다.
        
    - 즉, Seq2Seq 구조는 입력 데이터의 종류(오디오, 텍스트, 이미지)에 구애받지 않는 범용적인 프레임워크라는 점을 강조했습니다.


# 마지막 "절벽(Cliffhanger)": 4단어 vs 4000단어 🧱

강의의 가장 마지막 멘트입니다. 다음 강의(Attention)를 위한 빌드업이지만, 숫자를 들어 구체적으로 문제점을 지적했습니다.

- **강의 디테일:**
    
    - "입력이 **4단어**든 **4,000단어**든 상관없이, 인코더는 무조건 **똑같은 크기의 벡터 하나(One Little Vector)**에 모든 정보를 구겨 넣어야 한다."
        
    - **의미:** 이것이 왜 긴 문장 번역에서 성능이 급격히 떨어지는지에 대한 **물리적/용량적 한계**를 아주 직관적인 숫자로 비유했습니다.